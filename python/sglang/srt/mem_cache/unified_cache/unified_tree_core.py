...
class UnifiedTreeCore(UnifiedTreeCoreInterface):
    ...
    def probe_prefix_len(self, key: RadixKey) -> int:
        key = RadixKey(
            key.token_ids, key.extra_key, is_bigram=key.is_bigram or self.is_eagle
        ).page_aligned(self.page_size)
        if not len(key):
            return 0
        value, _, _, device_count = self._match_prefix_helper_readonly(key)
        return sum(len(chunk) for chunk in value[:device_count])

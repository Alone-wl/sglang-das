...
class EBNFComposer:
    ...
    def _handle_enum(prop: dict, function_format: str) -> str:
        ...
        def quote_terminal(value: str) -> str:
            return f'"{EBNFComposer._escape_ebnf_string(value)}"'

        def format_enum_val(v: Any) -> str:
            if prop_type == "boolean":
                if function_format == "json" or function_format == "xml":
                    return quote_terminal("true" if v else "false")
                elif function_format == "pythonic":
                    return quote_terminal("True" if v else "False")
                else:
                    return quote_terminal(str(v))  # fallback

            if prop_type == "string":
                if function_format == "xml":
                    return quote_terminal(str(v))
                else:  # json or pythonic
                    return quote_terminal(json.dumps(str(v), ensure_ascii=False))

            # All other types (number, integer, etc.)
            return quote_terminal(json.dumps(v, ensure_ascii=False))
    ...
    @staticmethod
    def _escape_ebnf_string(value: str) -> str:
        return (
            value.replace("\\", "\\\\")
            .replace('"', '\\"')
            .replace("\n", "\\n")
    ...
    def build_ebnf(
        ...
        ebnf_lines = [
            ...
            + " | ".join(
                [
                    f"call_{hash_string_to_short_hex(tool.function.name) if call_rule_fmt else tool.function.name}"
                    for tool in tools
                ]
            ),
        ]
        ...
        for tool in tools:
            ...
            namehash = (
                hash_string_to_short_hex(tool_name) if call_rule_fmt else tool_name
            )
            ebnf_lines.append(
                call_template.format(
                    name=tool_name,
                    arguments_rule=f"arguments_{namehash}",
                    namehash=namehash,
                )
            )

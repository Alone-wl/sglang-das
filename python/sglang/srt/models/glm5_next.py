...
class ModelNextLinearAttention(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        requested_fuse_qkvbfg = envs.SGLANG_GLM5_NEXT_FUSE_QKVBFG.get()
        ...
        if requested_fuse_qkvbfg and not fuse_qkvbfg_supported:
            log_info_on_rank0(
                logger,
                "SGLANG_GLM5_NEXT_FUSE_QKVBFG is disabled because ModelNext KDA "
...
class ModelNextForCausalLM(nn.Module):
    ...
    def forward_split_prefill(
        ...
        if start == 0:
            ...
            if dsa_use_prefill_cp(forward_batch, self.model.nsa_enable_prefill_cp):
                hidden_states = cp_plain_split(hidden_states)
                ...
                with ctx:
                    if i in self.model.layers_to_capture:
                        if self.config.mhc:
                            ...
                            aux_hidden_states.append(
                                get_parallel().attn_tp_group.all_gather(
                                    forward_batch.hidden_states

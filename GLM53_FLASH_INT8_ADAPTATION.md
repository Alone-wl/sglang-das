# GLM-5.3-Flash INT8/FP8 W8A8 适配交接

## 1. 文档用途

本文记录 `main_glm53_flash` 分支上的 GLM-5.3-Flash INT8 适配现状，供后续开发者直接继续调试。内容以仓库代码和提交记录为准；未实际验证的结论必须标为“待验证”，不得按已完成处理。

本文以已完成的 INT8（W8A8）路径为基线，继续记录目标 checkpoint `/home/work/GLM-5.3-Flash-Channel-FP8-w8a8` 的 FP8 W8A8（FP8 权重、FP8 激活）适配。这里的 W8A8 只表示权重和激活均为 8-bit，不等同于 INT8；FP8 KV cache 仍是独立开关，不能由 FP8 W8A8 结论推导。
...
- 代码阅读和修改在本地完成；本地提交并推送远端后，在服务器容器中拉取对应远端提交，再进行验证。不得以容器已有的其他提交代替本轮同步。
